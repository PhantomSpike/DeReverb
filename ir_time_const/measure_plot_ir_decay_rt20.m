...
rt_20.small = -20./b.small(:,2);
rt_20.large = -20./b.large(:,2);
rt_20.freqs = freqs;
    ...
    for s = 1:size(coch,2)
        ...
        for f = 1:n_f
            ...
            annotation(gcf,'textbox',pos{f} + [xdiff*Apos_x, ydiff*Apos_y -pos{f}(3)*(1-Asz_x) -pos{f}(4)*(1-Asz_y)],...
                'String', sprintf('RT_{20}=%.1fs\nR^{2}=%.1f',rt_20.(room)(f),r.(room)(f)),'LineStyle','none','FontSize',axis_sz,'FontWeight','Normal','Color','r');
...
%% Plot RT20 vs freqeuncy
lw = 6;
ix = rt_20.freqs>freq_down_bound & rt_20.freqs<freq_up_bound;
plot_freqs = fliplr(rt_20.freqs(ix));
...
plot_rt20_small = flipud(rt_20.small(ix));
plot_rt20_large = flipud(rt_20.large(ix));

figure('units','normalized','outerposition',[0 0 1 1],'DefaultTextFontName', font_type, 'DefaultAxesFontName',font_type);
semilogx(plot_freqs,plot_rt20_small,'Color',small_color,'LineWidth',lw);hold on;
semilogx(plot_freqs,plot_rt20_large,'Color',big_color,'LineWidth',lw);
hold off;
...
set(gcf,'color','w');
set(gca,'TickDir','out');
save_name = fullfile(save_dir,['RT20 vs BF individual rooms.svg']);
saveas(gcf,save_name);
close;
...
y_small_neg = plot_rt20_small; y_small_neg = y_small_neg(:);
y_big_neg = plot_rt20_large; y_big_neg = y_big_neg(:);
...
figure('units','normalized','outerposition',[0 0 1 1],'DefaultTextFontName', font_type, 'DefaultAxesFontName',font_type);
plot(log10(plot_freqs),plot_rt20_small,'-','Color',small_color,'LineWidth',lw);hold on;
plot(log10(plot_freqs),plot_rt20_large,'-','Color',big_color,'LineWidth',lw);
set(gca, 'XScale', 'log');
...
set(gcf,'color','w');
set(gca,'TickDir','out');
save_name = fullfile(save_dir,['Regression RT20 vs BF individual rooms.svg']);
saveas(gcf,save_name);
close;
