...
ir_med_path = '/mnt/40086D4C086D41D0/Reverb_normative/IRs/Full_IRs/ferret_medium_full.mat';
ir_large_path = '/mnt/40086D4C086D41D0/Reverb_normative/IRs/Full_IRs_calc/ferret_large_full.mat';
save_dir = '/mnt/40086D4C086D41D0/Reverb_paper/fig_4_test';
ear = 'right';
...
data_med = load(ir_med_path);
data_large = load(ir_large_path);
fs = data_small.Fs;
switch ear
    case 'left'
        ir.small = data_small.data(:,1); ir.med = data_med.data(:,1); ir.large = data_large.data(:,1);
    case 'right'
        ir.small = data_small.data(:,2); ir.med = data_med.data(:,2); ir.large = data_large.data(:,2);
end

dt_s = 1/fs;
l_small_s = length(ir.small)/fs;
l_med_s = length(ir.med)/fs;
l_large_s = length(ir.large)/fs;
t_small = [dt_s:dt_s:l_small_s];
t_med = [dt_s:dt_s:l_med_s];
...
%This is an arbitrary choice, need to do this time cut-off in a more
%principled way
end_s.small = 0.3;
end_s.med = 0.76;
end_s.large = 1.5;
...
for s = 1:n_cond
    ...
    for f = 1:n_f
        ...
        %Compute the Hilbert transform and the enevelope of each band using
        %the 'analytic' signal
%         envelope_type = 'analytic';
%         np=10;
%         [up_env,low_env] = envelope(x_f,np,envelope_type);
%         plot(t_fit_s,x_f); hold on; plot(t_fit_s,up_env); plot(t_fit_s,low_env);
%         pause(0.75);
%         close;
        X = [ones(length(t_fit_s),1),t_fit_s(:)]; %Make design matrix
        y = x_f(:);
%         envel.up.(room)(f,:) = up_env;
%         envel.low.(room)(f,:) = low_env;
...
rt_10.small = -10./b.small(:,2);
rt_10.med = -10./b.med(:,2);
rt_10.large = -10./b.large(:,2);
rt_10.freqs = freqs;

%% Calaculate Global RT10 values
rt_10_global.small = median(rt_10.small);
rt_10_global.med = median(rt_10.med);
rt_10_global.large = median(rt_10.large);

%% Plot the decay profiles of each freqeuncy band with envelope
...
%         figure('units','normalized','outerposition',[0 0 1 1],'DefaultTextFontName', font_type, 'DefaultAxesFontName',font_type);
%         room = coch(s).reverb_cond;
%         ix_length = length(envel.up.(room)(1,:));
%         for f = 1:n_f
%             subplot('position',pos{f});
%             plot(coch(s).t,coch(s).X_ft(f,:),'LineWidth',lw);
%             hold on;
%             plot(coch(s).t(1:ix_length),envel.up.(room)(f,:),'LineWidth',lw);
%             plot(coch(s).t(1:ix_length),envel.low.(room)(f,:),'LineWidth',lw);
...
%                 'String', sprintf('RT_{10}=%.1fs\nR^{2}=%.1f',rt_10.(room)(f),r.(room)(f)),'LineStyle','none','FontSize',axis_sz,'FontWeight','Normal','Color','r');
%             
%             set(gcf,'color','w');
%             set(gca,'TickDir','out');
%             ylim([-40 0])
...
%% Plot the decay profiles of each freqeuncy band w/o envelope
if plot_decay_profiles
    %Params for subplots title textbox
    tpos_x = 0.4; tpos_y = 0.9; tsz_x = 0.3; tsz_y = 0.3;
    %Params for RT60, R^2 textbox
...
%% Plot RT10 vs freqeuncy
lw = 6;
ix = rt_10.freqs>freq_down_bound & rt_10.freqs<freq_up_bound;
plot_freqs = fliplr(rt_10.freqs(ix));
...
%Convert to ms
plot_rt10_small = 1000*flipud(rt_10.small(ix));
plot_rt10_med = 1000*flipud(rt_10.med(ix));
plot_rt10_large = 1000*flipud(rt_10.large(ix));

figure('units','normalized','outerposition',[0 0 1 1],'DefaultTextFontName', font_type, 'DefaultAxesFontName',font_type);
semilogx(plot_freqs,plot_rt10_small,'Color',small_color,'LineWidth',lw);hold on;
semilogx(plot_freqs,plot_rt10_med,'Color',med_color,'LineWidth',lw);
semilogx(plot_freqs,plot_rt10_large,'Color',big_color,'LineWidth',lw);
hold off;
...
annotation('textbox',[0.72 0.75 0.1 0.1],'String', sprintf('Medium room'),'LineStyle','none','Color',med_color,'FontSize',legend_font_sz,'FontWeight','Normal');
annotation('textbox',[0.72 0.7 0.1 0.1],'String', sprintf('Small room'),'LineStyle','none','Color',small_color,'FontSize',legend_font_sz,'FontWeight','Normal');
...
set(gcf,'color','w');
set(gca,'TickDir','out');
save_name = fullfile(save_dir,['RT10 vs BF individual rooms.svg']);
saveas(gcf,save_name);
close;
...
X_med = [ones(n_reg,1),log10(plot_freqs)];
X_big = [ones(n_reg,1),log10(plot_freqs)];

y_small_neg = plot_rt10_small; y_small_neg = y_small_neg(:);
y_med_neg = plot_rt10_med; y_med_neg = y_med_neg(:);
y_big_neg = plot_rt10_large; y_big_neg = y_big_neg(:);

[b_small,~,~,~,stats_small] = regress(y_small_neg,X_small);
[b_med,~,~,~,stats_med] = regress(y_med_neg,X_med);
[b_big,~,~,~,stats_big] = regress(y_big_neg,X_big);
r_small = sign(b_small(2))*sqrt(stats_small(1)); p_small = stats_small(3);
r_med = sign(b_med(2))*sqrt(stats_med(1)); p_med = stats_med(3);
r_big = sign(b_big(2))*sqrt(stats_big(1)); p_big = stats_big(3);

figure('units','normalized','outerposition',[0 0 1 1],'DefaultTextFontName', font_type, 'DefaultAxesFontName',font_type);
plot(log10(plot_freqs),plot_rt10_small,'-','Color',small_color,'LineWidth',lw);hold on;
plot(log10(plot_freqs),plot_rt10_med,'-','Color',med_color,'LineWidth',lw);
plot(log10(plot_freqs),plot_rt10_large,'-','Color',big_color,'LineWidth',lw);
set(gca, 'XScale', 'log');
h1 = refline([b_small(2),b_small(1)]); h1.Color = small_color; h1.LineWidth = lw; h1.LineStyle = '--';
h2 = refline([b_med(2),b_med(1)]); h2.Color = med_color; h2.LineWidth = lw; h2.LineStyle = '--';
h3 = refline([b_big(2),b_big(1)]); h3.Color = big_color; h3.LineWidth = lw; h3.LineStyle = '--';
hold off;
...
annotation('textbox',[0.6 0.72 0.1 0.1],'String', sprintf('r_{med}= %.2f^{%s}',r_med,p_star(p_med)),'LineStyle','none','Color',med_color,'FontSize',legend_font_sz,'FontWeight','Normal');
annotation('textbox',[0.6 0.62 0.1 0.1],'String', sprintf('r_{small}= %.2f^{%s}',r_small,p_star(p_big)),'LineStyle','none','Color',small_color,'FontSize',legend_font_sz,'FontWeight','Normal');
set(gca,'FontSize',all_font_sz,'FontWeight','Normal');
set(gcf,'color','w');
set(gca,'TickDir','out');
save_name = fullfile(save_dir,['Regression RT10 vs BF individual rooms.svg']);
saveas(gcf,save_name);
close;
