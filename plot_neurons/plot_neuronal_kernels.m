function plot_neuronal_kernels(kernels,save_dir,ker_per_plot, animal_names)
...
r_type2{1} = 'small';r_type2{2} = 'med';r_type2{3} = 'big';
...
for g = 1:n_groups
    ...
    for r = 1:n_cond
        ...
        for k = 1:ker_per_plot
            ...
            switch model
                case {'sep','sep_kh'}
                    if strcmp(animal_names{c},'Noah') || strcmp(animal_names{c},'Derekah')
                        k_fh = kernels{c}.kernel.(r_type2{r}).k_f*fliplr(kernels{c}.kernel.(r_type2{r}).k_h');
                    else
                        k_fh = kernels{c}.kernel.(r_type{r}).k_f*fliplr(kernels{c}.kernel.(r_type{r}).k_h');
                    end
                case {'lasso','ridge','elastic'}
                    if strcmp(animal_names{c},'Noah') || strcmp(animal_names{c},'Derekah')
                        k_fh = fliplr(kernels{c}.kernel.(r_type2{r}).main{end}.k_fh);
                    else
