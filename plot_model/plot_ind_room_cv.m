function fig = plot_ind_room_cv(kernels,params,room_sz)
...
k_folds = length(kernels{1}.main)-1;
for kf = 1:k_folds
    %% STRF - k_fh
    ...
    for k = 1:n_f
        switch model_type
            ...
            case {'ridge','lasso','manual_alpha','elastic'}
                k_fh = fliplr(kernels{k}.main{kf}.k_fh); %Take the full kernel for the elent case
        end
    ...
    save_dir_full = fullfile(save_dir,['part_',num2str(kf)]);
    if ~exist(save_dir_full,'dir')
        mkdir(save_dir_full);
    end
    save_name = fullfile(save_dir_full,strjoin({params.coch_type,params.model,params.normz,room_sz,'kfh.png'},'_'));
    ...
    sgtitle([coch_type,' ',model_type,' model norm ',normz,' ',room_sz,' room'],'FontSize',title_sz,'Color','k','FontWeight','Bold');
    save_name = fullfile(save_dir_full,strjoin({params.coch_type,params.model,params.normz,room_sz,'kh.png'},'_'));
    export_fig(save_name);
    close all;
