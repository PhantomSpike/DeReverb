function plot_model_tconst_grande_cv(kernel_dir)
%% Params
...
for kf = 1:k_folds
    ...
    for k = 1:n_ker
        for r = 1:n_rooms
            ...
            if plot_hyperparams
                alpha(k).(room) = kernels.(room){k}.main{kf}.alpha;
                lambda(k).(room) = kernels.(room){k}.main{kf}.lambda;
            end
        end
    ...
    set(gcf,'color','w');
    hold off;
    save_name = fullfile(save_dir_full,['Histogram of per change for normative ',model,' kernel.png']);
    export_fig(save_name);
    close all;
    ...
    set(gcf,'color','w');
    hold off;
    save_name = fullfile(save_dir_full,['Histogram of tau ms change for normative ',model,' kernel.png']);
    export_fig(save_name);
    close all;
