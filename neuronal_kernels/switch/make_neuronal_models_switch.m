function make_neuronal_models_switch(model,h_max_ms,dt_ms,normz,skip_ms,save_dir,kfold,n_cores)
%% Input params
...
parfor c = 1:n_clusters
    ...
    if ismember(temp.data.params.animal_name,["Cork","Kilkenny","Derry"])
        kernels{c} =  gen_neuronal_kernel_switch(coch1,temp.data,h_max_ms,model,skip_ms,normz,kfold);
    elseif ismember(temp.data.params.animal_name,["Noah","Derekah"])
        kernels{c} =  gen_neuronal_kernel_switch(coch2,temp.data,h_max_ms,model,skip_ms,normz,kfold);
    else
