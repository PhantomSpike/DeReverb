...
cluster_dir = '/mnt/40086D4C086D41D0/Reverb_neuronal_data/Kernel_fits/kfolds/LNP_model_noneuron_norm/perfreq_noneuro/ridge/10ms/Predicted_PSTHs_noneuro_norm'; %Directory with all the clusters
...
parfor c = 1:n_clusters
    ...
    if ismember(animal_names{c},["Ronnie","PLP"])
        kernels{c} =  gen_simlnp_kernel(coch1, temp.psth, h_max_ms,model, normz, kfold, animal_names{c});
    elseif ismember(animal_names{c},["Cork","Kilkenny","Derry"])
        kernels{c} =  gen_simlnp_kernel(coch2, temp.psth, h_max_ms,model, normz, kfold, animal_names{c});
    elseif ismember(animal_names{c},["Noah","Derekah"])
        kernels{c} =  gen_simlnp_kernel(coch3, temp.psth, h_max_ms,model, normz, kfold, animal_names{c});
    else
...
lnp_dir = fullfile(save_dir,'LNP_model_sim_noneuron_norm');
